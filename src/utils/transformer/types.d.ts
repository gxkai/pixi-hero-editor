declare module "@pixi/display" {
    export { Bounds, DisplayObject, Container } from "pixi.js";
}

declare module "@pixi/graphics" {
    export {
        LINE_JOIN,
        LINE_CAP,
        Graphics,
        GraphicsData,
        GraphicsGeometry,
        GRAPHICS_CURVES,
        LineStyle,
        SHAPES,
        Polygon,
    } from "pixi.js";
}

declare module "@pixi/math" {
    export {
        IPointData,
        Point,
        Matrix,
        Transform,
        Rectangle,
        SHAPES,
        RoundedRectangle,
        Ellipse,
        Circle,
        Polygon,
    } from "pixi.js";
}

declare module "@pixi/interaction" {
    export { InteractionEvent } from "pixi.js";
}

declare module "@pixi/sprite" {
    export { Sprite } from "pixi.js";
}

declare module "@pixi/text" {
    export { Text, TextMetrics, TextStyle } from "pixi.js";
}

declare module "@pixi/text-bitmap" {
    export { BitmapFont, BitmapText } from "pixi.js";
}
